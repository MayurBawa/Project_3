(() => {
  ...
  function showScene(index) {
    ...
    if (current === 8 && activeMusic !== "bgMusic") {
  ...
  function updateBirthdayCountdown() {
    const targetBaseYear = new Date().getFullYear();
    let target = new Date(targetBaseYear, 8, 7);

    if (target <= new Date()) {
      target = new Date(targetBaseYear + 1, 8, 7);
    }
    ...
    const newAge = target.getFullYear() - birthDate.getFullYear();

    document.getElementById("birthdayCountdown").textContent =
      `${days}d ${String(hours).padStart(2, "0")}h ${String(minutes).padStart(2, "0")}m ${String(seconds).padStart(2, "0")}s until you turn ${newAge} ✨`;
  }
  ...
  function startPetals() {
    ...
    const layer = document.querySelector(".cake-scene");
